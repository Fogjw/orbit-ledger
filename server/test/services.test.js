...
// 收入与支出共用 category 维度（靠名字「收入」前缀区分），维度机制也完全一致：
// 每个维度每笔都要有归属，只是收入的缺省占位叫「收入·未分类」而不是「未分类」，
// 两者并存、分别收纳（前缀还让它按收入类目渲染成四角星）。
describe('收入的缺省占位与支出分开，但维度机制一致', () => {
  ...
  test('收入不选任何 tag：每个维度都挂「收入·未分类」', () => {
    ...
    assert.deepEqual(primaries.map(x => x.dim).sort(), ['category', 'context'], '收入每个维度都有归属');
    assert.ok(primaries.every(x => x.name === '收入·未分类'), '缺省占位一律是「收入·未分类」');
    assert.ok(primaries.every(x => x.is_unnamed === 1), '它仍是不可改名/改色的占位 tag');
  });
  ...
  test('支出不选任何 tag：品类与情境各挂一个「未分类」，与收入互不串门', () => {
    ...
    // 同一账本里再记一笔收入：同名的占位在两个维度里各存一份，收支互不串门
    svc.expenses.add({ ledgerId: l.id, type: 'income', amountCents: 100, date: '2026-09-06' });
    const unnamedByDim = (dimKey) => db.prepare(`
      SELECT t.name FROM tags t JOIN dimensions d ON d.id = t.dimension_id
       WHERE t.ledger_id = ? AND d.key = ? AND t.is_unnamed = 1 AND t.parent_tag_id IS NULL`).all(l.id, dimKey).map(r => r.name);
    assert.deepEqual(new Set(unnamedByDim('category')), new Set(['未分类', '收入·未分类']), '品类维：两个占位并存');
    assert.deepEqual(new Set(unnamedByDim('context')), new Set(['未分类', '收入·未分类']), '情境维：同样两个占位并存');
  });

  test('收入也能选情境：传了就落库，和支出走同一套维度机制', () => {
    const { db, svc } = setup();
    ...
    const ctx = linksOf(db, e.id).filter(x => x.dim === 'context' && x.role === 'primary');
    assert.equal(ctx.length, 1, '情境维恰一个 primary');
    assert.equal(ctx[0].name, '和朋友');
  });
});
