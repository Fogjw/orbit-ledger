...
> 本文档是 `server/`（业务逻辑层）的分层、数据模型与 REST / MCP 契约的权威来源；
> 前端（`web/`）的交互与视觉约定见其源码，桌面壳层见 `electron/`。
...
- `README.md` —— 项目定位、运行与打包方式
- `web/` —— 前端实现（零依赖 Canvas2D 星图 / 星轨 / 浮层交互）
- `electron/` —— 桌面壳层与安装包配置（根 `package.json` 的 `build` 段）
